...
const IM_CHART_DIALOG = {
  ...
  mutations: { // 修改状态
    ...
    PUSH_IM_CHART_CHATLIST(state, user) {
      let chatList=state.chatList||[];
      var bool=true;
      for(var i=0;i<chatList.length;i++){
        if(chatList[i].id==user.id) bool=false;
      }
      bool && chatList.unshift(user);
      state.chatList=chatList;
      localStorage.setItem('chatList',JSON.stringify(state.chatList))
    },
    SET_IM_CHART_CHATLIST(state, value) {
    ...
    PUSH_IM_CHART_DIALOG_GROUP_MESSAGE(state, {message, groupId}) { // 用户聊天信息
      let chat = state.groupMessageMap[groupId] || []
      chat.push(message)
      Vue.set(state.groupMessageMap, groupId, chat)
      Vue.set(state,'groupMessageMap',state.groupMessageMap);
    },
    ...
    SET_IM_GROUP_MESSAGE_MAP(state,value){
      state.groupMessageMap=value;
      Vue.set(state,'groupMessageMap',value);
    }
  },
  actions: { // 调用方法（接收传进来的参数）
    ...
    // 聊天列表
    PUSH_IM_CHART_CHATLIST({commit}, user) {
      commit('PUSH_IM_CHART_CHATLIST', user)
    },
    // 群组列表
    ...
    SET_IM_GROUP_MESSAGE_MAP({commit}, {value}){
      console.log(value)
      commit('SET_IM_GROUP_MESSAGE_MAP', {value})
    }
  },
  getters: { // 弹窗展示
      ...
      if (temp) {
        switch (temp.fileType) {
          ...
          case 'image':
            return '[图片]'
          default :
            return temp.data.replace(/(\<img.*?\>)/g, "[图片]")
        }
      }else{
        return "";
      }
